...
const INITIAL_USERS = [
  { id: 'u1', name: 'Ravi (Owner)', role: ROLES.ADMIN },
  { id: 'u2', name: 'Priya (Lead TL)', role: ROLES.LEAD_TL },
  { id: 'u3', name: 'Amit (Pre-Sales)', role: ROLES.PRE_SALES },
  { id: 'u4', name: 'Rahul (BDM 1)', role: ROLES.BDM },
  { id: 'u5', name: 'Sneha (BDM 2)', role: ROLES.BDM },
];
...
const initialLeads = [
  ...
  },
  {
    ...
    status: 'NEW LEAD', stage: null, assignedTo: null, addedBy: 'u3',
    expectedValue: 0, probability: 0,
    createdAt: new Date().toISOString(),
    nextFollowUp: null,
    timeline: [{ date: new Date().toISOString(), action: 'Lead added', user: 'Amit (Pre-Sales)' }],
    communications: []
  }
];
      ...
      <header className="h-16 bg-white border-b border-slate-200 flex items-center justify-between px-6 sticky top-0 z-20">
        ...
        <div className="flex items-center space-x-6">
          ...
          <div className="flex items-center space-x-3 border-l border-slate-200 pl-6">
            ...
            <button onClick={() => { setIsLoggedIn(false); setCurrentUser(null); }} className="ml-4 p-2 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Log Out">
              <LogOut size={18} />
            </button>
          </div>
        </div>
    ...
    return (
      <div className="min-h-screen bg-slate-50 flex items-center justify-center p-4">
        <div className="bg-white p-8 rounded-2xl shadow-xl w-full max-w-md border border-slate-200">
          ...
          <p className="text-center text-slate-500 mb-8 text-sm">Sign in to access your secure dashboard.</p>
          <div className="space-y-4">
            <p className="text-xs font-semibold text-slate-400 uppercase tracking-wider mb-2">Select Account Setup (Testing):</p>
            {users.map(u => (
              <button key={u.id} onClick={() => { setCurrentUser(u); setIsLoggedIn(true); setCurrentView('dashboard'); }} className="w-full flex items-center justify-between p-4 border border-slate-200 rounded-xl hover:border-blue-500 hover:bg-blue-50 transition-all text-left group">
                <div><div className="font-bold text-slate-800 group-hover:text-blue-700">{u.name}</div><div className="text-xs text-slate-500 font-medium">{u.role}</div></div>
